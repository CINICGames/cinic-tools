...
public static class Git {
    public static string RunGitCommand(string gitCommand) {
        ...
        // Log any errors.
        if (errorOutput != "") {
            UnityEngine.Debug.LogWarning("Git: " + errorOutput);
        }
    ...
    public static void UpdateBundleVersion(bool isMajor, bool isMinor, bool isPatch) {
        ...
        try {
            ...
            UnityEngine.Debug.LogErrorFormat("Unexpected bundleVersion Format: {0}\nExpected format: *.*.* ", PlayerSettings.bundleVersion);
            throw e;
        }
        if (isMajor) {
            major++;
        ...
        UnityEngine.Debug.LogFormat("Updated Game Version: {0}.{1}.{2}", major, minor, patch);
