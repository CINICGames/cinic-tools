	...
	public partial class SpreadsheetManager {
		...
		static IEnumerator CheckForRefreshToken() {
			if (Application.isPlaying) {
				yield return new Task(GoogleAuthorisationHelper.CheckForRefreshOfToken());
			}
#if UNITY_EDITOR
			else {
				yield return EditorCoroutineRunner.StartCoroutine(GoogleAuthorisationHelper.CheckForRefreshOfToken());
			}
#endif
		}
			...
			using (request) {
				...
				if (callback != null) {
					try {
						...
						Debug.LogWarning("Authentication Token Refreshed");
					}
				}
			}
		}
